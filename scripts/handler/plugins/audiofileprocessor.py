...
def parse_instrument(instrumenttrack_element):
    ...
    if instrument is not None:
        ...
        instrument_info['plugin_name'] = instrument_name

        plugin_name = instrument_name.lower()
        plugin_element = instrument.find(f'.//{plugin_name}')
        if plugin_element is not None:
            instrument_info[plugin_name] = {
                key: value for key, value in plugin_element.attrib.items()
            }
